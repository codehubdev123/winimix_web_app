"use client";
import Image from "next/image";
import { Heart, Moon, Search, ShoppingCart, UserRound } from "lucide-react";
import HeaderIconLink from "./HeaderIconLink";
import { useTheme } from "@/contexts/ThemeContext";

const TopHeader = () => {
  const { theme, setTheme, isDark } = useTheme();

  const toggleTheme = () => {
    setTheme(isDark ? "light" : "dark");
  };

  const setSystemTheme = () => {
    setTheme("system");
  };

  return (
    <div className="bg-primary dark:bg-header-dark">
      <div className="max-w-7xl mx-auto  h-[88px] flex items-center justify-between px-5 lg:px-0">
        <div className="md:text-2xl font-bold text-white">
          <Image src="/logo.svg" width={150} height={48} alt="logo" />
        </div>
        <div className="flex-1 px-12">
          <div className="flex px-4 py-3  border-1 border-white overflow-hidden h-[48px] mx-auto rounded-[100px] hidden lg:flex">
            <Search className="w-5 h-5 text-white mr-3 rtl:ml-3 rotate-90 " />
            {/* <svg */}
            {/*   xmlns="http://www.w3.org/2000/svg" */}
            {/*   viewBox="0 0 192.904 192.904" */}
            {/*   width="16px" */}
            {/*   className="fill-gray-600 mr-3 rotate-90" */}
            {/* > */}
            {/*   <path d="m190.707 180.101-47.078-47.077c11.702-14.072 18.752-32.142 18.752-51.831C162.381 36.423 125.959 0 81.191 0 36.422 0 0 36.423 0 81.193c0 44.767 36.422 81.187 81.191 81.187 19.688 0 37.759-7.049 51.831-18.751l47.079 47.078a7.474 7.474 0 0 0 5.303 2.197 7.498 7.498 0 0 0 5.303-12.803zM15 81.193C15 44.694 44.693 15 81.191 15c36.497 0 66.189 29.694 66.189 66.193 0 36.496-29.692 66.187-66.189 66.187C44.693 147.38 15 117.689 15 81.193z"></path> */}
            {/* </svg> */}
            <input
              type="text"
              placeholder="Search Something..."
              className="w-full outline-none bg-transparent text-white text-sm placeholder-white/70"
            />
          </div>
        </div>
        <div className="flex items-center justify-between gap-6">
          <div className="hidden lg:block">
            <Image
              src="/sale.svg"
              width={176}
              height={48}
              className="max-w-[176px] h-[48px]"
              style={{ objectFit: "contain" }}
              alt="Limited Time Sale"
            />
          </div>
          <div className="flex items-center  gap-5">
            <HeaderIconLink icon={<Moon />} onClick={toggleTheme} />
            <HeaderIconLink icon={<Heart />} />
            <HeaderIconLink icon={<UserRound />} />
            <HeaderIconLink icon={<ShoppingCart />} isCart isLink />
          </div>
        </div>
      </div>
    </div>
  );
};

export default TopHeader;
